...
Cmax = [Q1max/sqrt(H1max); Q2max/sqrt(H2max); Q3max/sqrt(H3max)];
Usp = [qs; C1s; C2s; C3s];
xsp = [H1sp; H2sp; H3sp];
x0 = [H10; H20; H30];
U_h = zeros(4*Nc,1);
...
for i = 1:Nc
    for j = 1:i
        Acon(4*i-3:4*i,4*j-3:4*j) = eye(4);
    end
end
Acon = [Acon; -Acon];
x0_con = 10^(-5)*ones(4*Nc,1);

for i = 1:length(t1)-1
    b1con = repmat(([qmax; Cmax(1); Cmax(2); Cmax(3)]-U),Nc,1);
    b2con = repmat(U,Nc,1);
    bcon = [b1con; b2con];
    
    %Optimal Control Action under System Constraints
    options = optimoptions('fmincon', 'Algorithm', 'sqp');
    [DU_dot, Jval, exitflag] = fmincon(@(DU)obj_fun(DU,Phi,F,Rw,Y_dotsp,xtot_dot), x0_con, Acon, bcon, [], [], [], [], [], options);
    for k = 1:Nc
        if k == 1
