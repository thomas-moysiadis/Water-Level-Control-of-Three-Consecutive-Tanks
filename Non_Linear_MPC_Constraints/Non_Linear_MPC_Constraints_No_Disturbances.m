...
for i = 1:length(t1)-1
    ...
    nlobj.States(1).Max = H1max;
    nlobj.States(2).Min = 0;
    nlobj.States(2).Max = H2max;
    nlobj.States(3).Min = 0;
    nlobj.States(3).Max = H3max;
    nlobj.ManipulatedVariables(1).Min = 0;
    nlobj.ManipulatedVariables(1).Max = qmax;
    nlobj.ManipulatedVariables(2).Min = 0;
    nlobj.ManipulatedVariables(2).Max = Q1max / sqrt(0.35);
    nlobj.ManipulatedVariables(3).Min = 0;
    nlobj.ManipulatedVariables(3).Max = Q2max / sqrt(0.35);
    nlobj.ManipulatedVariables(4).Min = 0;
    nlobj.ManipulatedVariables(4).Max = Q3max / sqrt(0.35);
    ...
    %Optimal Control Action under System Constraints
